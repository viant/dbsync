...
//Session1 represents a ssession
...
func (s *Session1) IsDebug() bool {
	return s.Config.Debug || s.Request.Debug
}

//SetError sets an error
func (s *Session1) SetError(err error) bool {
...
func (s *Session1) SetStatus(status string) {
...
func (s *Session1) IsClosed() bool {
	return atomic.LoadUint32(&s.closed) == 1
}

//SetSynMethod sets sync1 method
func (s *Session1) SetSynMethod(method string) {
...
func (s *Session1) hasOnlyDataAppend(sourceData, destData Record, criteria map[string]interface{}, status *Info) bool {
...
func (s *Session1) findInSyncMaxID(idRange *IDRange, narrowedCriteria map[string]interface{}, status *Info) (int, error) {
...
func (s *Session1) runDiffSQL(criteria map[string]interface{}, source, dest *[]Record) ([]string, error) {
	...
	s.Log(nil, fmt.Sprintf("DiffSignature SQL(%v) src:%v\ndst:%v\n\tsrc:%v\n\tdst:%v", criteria, sourceSQL, destDQL, source, dest))
...
func (s *Session1) sumRowCount(records Records) int {
	return records.Sum(s.Builder.countColumnAlias)
}

func (s *Session1) maxValue(records Records) int {
	var result = 0
	if len(records) == 0 {
		return result
	}
...
func (s *Session1) sumRowDistinctCount(records Records) int {
	return records.Sum(s.Builder.uniqueCountAlias)
}

func (s *Session1) sumRowNotNullDistinctSum(records Records) int {
	return records.Sum(s.Builder.uniqueNotNullSumtAlias)
}

func (s *Session1) recordsSum(data []Record, column string) int {
	result := 0
...
func (s *Session1) setInfoRange(source, dest map[string]interface{}, info *Info) {
	if !s.hasID {
		return
	}
...
func (s *Session1) isMergeDeleteStrategy(source, dest map[string]interface{}) bool {
...
func (s *Session1) validateSourceData(sourceRecords []Record, criteria map[string]interface{}) error {
	if !s.hasID {
		return nil
	}
...
func (s *Session1) validateDestinationData(destRecords []Record, criteria map[string]interface{}) error {
	if !s.hasID {
		return nil
	}
...
func (s *Session1) buildSyncInfo(sourceRecords, destRecords []Record, groupColumns []string, criteria map[string]interface{}, optimizeAppend bool) (*Info, error) {
	var err error
	result := &Info{}
	defer func() {
		if !result.InSync {
			s.Log(nil, fmt.Sprintf("sync1 method: %v, %v", result.Method, criteria))
		}
	}()
	...
	if s.IsDebug() {
		s.Log(nil, fmt.Sprintf("out of sync1: %v\n\tsrc:%v\n\tdst:%v\n", criteria, sourceRecords, destRecords))
	}
...
//GetSyncInfo returns a sync1 info
func (s *Session1) GetSyncInfo(criteria map[string]interface{}, optimizeAppend bool) (*Info, error) {
...
func (s *Session1) readSyncInfoBatch(batchCriteria map[string]interface{}, index *indexedRecords) error {
	var sourceData = make([]Record, 0)
	var destData = make([]Record, 0)
...
//BatchSyncInfo returns batch sync1 info
func (s *Session1) BatchSyncInfo() error {
	var err error
	...
	if matched == 0 && len(batchedCriteria) > 0 {
		...
		s.Error(nil, fmt.Sprintf("warning: invalid partition expression - unable to match in batch sync1 status with keys: %v, \n actual: %v", keys, actualKeys))
	}
	return err
}

//Log logs session message
func (s *Session1) Log(partition *Partition, message string) {
	if !s.IsDebug() {
		return
	}
	suffix := ""
...
func (s *Session1) Error(partition *Partition, message string) {
	if partition == nil {
...
func (s *Session1) buildTransferJob(partition *Partition, criteria map[string]interface{}, suffix string, sourceCount, destCount int) *TransferJob {
...
func (s *Session1) Close() {
	_ = s.SourceDB.ConnectionProvider().Close()
	_ = s.DestDB.ConnectionProvider().Close()
}

//NewSession1 creates a new session
func NewSession1(request *Request, response *Response, config *Config) (*Session1, error) {
	...
	var session = &Session1{
		...
		DestDB:             destDB,
		Builder:            builder,
		isBatchedPartition: request.Partition.SyncMode == shared.SyncModeBatch && request.Partition.ProviderSQL != "",
		isBatchedChunk:     request.Chunk.SyncMode == shared.SyncModeBatch,
		isChunkedTransfer:  request.Chunk.Size > 0,
		isPartitioned:      request.Partition.ProviderSQL != "",
		mux:                &sync.Mutex{},
		differ:             &differ1{Builder: builder},
	}
	...
	if session.isChunkedTransfer && !session.isBatchedChunk && session.isBatchedPartition {
		return nil, fmt.Errorf("batchedPartitionStatus can not run with individual chunk sync1")
	}
	return session, nil
}
